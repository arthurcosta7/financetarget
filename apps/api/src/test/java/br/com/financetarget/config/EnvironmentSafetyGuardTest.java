...
class EnvironmentSafetyGuardTest {
    ...
    void acceptsAClosedStagingConfiguration() {
        assertThatNoException().isThrownBy(() -> EnvironmentSafetyGuard.validate("staging",
                new CorsProperties(List.of("https://staging.example.test")), auth(true), DISABLED, resend(false)));
    }
    ...
    void rejectsInsecureCookiesLocalOriginsAndIntegrationFlagsInStaging() {
        assertThatIllegalStateException().isThrownBy(() -> EnvironmentSafetyGuard.validate("staging",
                new CorsProperties(List.of("https://staging.example.test")), auth(false), DISABLED, resend(false)));
        assertThatIllegalStateException().isThrownBy(() -> EnvironmentSafetyGuard.validate("staging",
                new CorsProperties(List.of("http://localhost:3000")), auth(true), DISABLED, resend(false)));
        var enabled = new FeatureFlagProperties(true, false, false, false, false, false, false);
        assertThatIllegalStateException().isThrownBy(() -> EnvironmentSafetyGuard.validate("staging",
                new CorsProperties(List.of("https://staging.example.test")), auth(true), enabled, resend(false)));
    }

    @Test
    void requiresSafeResendConfigurationWhenEnabled() {
        assertThatNoException().isThrownBy(() -> EnvironmentSafetyGuard.validate("staging",
                new CorsProperties(List.of("https://staging.example.test")), auth(true), DISABLED, resend(true)));
        ...
        assertThatIllegalStateException().isThrownBy(() -> EnvironmentSafetyGuard.validate("staging",
                new CorsProperties(List.of("https://staging.example.test")), auth(true), DISABLED, invalid));
    }
